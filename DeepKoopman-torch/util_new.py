...
def load_data_with_ctrl(data_type, state_sqs: list = None):
    ...
    for s in STATE_VALUE:
        ...
        if s == 'NGSIM/NGSIM_US101_Density_Data_guiyihua':
    ...
    states = states.T
    print(states.shape)
    data_tensor = torch.tensor(states, dtype=torch.float).to(DEVICE)
